...
`Composition API` 使用 `hooks` 可以抽离逻辑到外部，但在使用时需要注意钩子函数内部的状态，在一个地方使用这个钩子函数并取出需要的变量，那么这些变量的状态变化是相关联的，而在另一个地方使用这个钩子函数取出的变量，其状态是和第一次调用钩子函数取出的变量状态无关的

说白了就是要注意多次调用钩子函数时其变量作用域的问题


### 事件修饰符

事件修饰符会按照顺序执行，需要注意顺序引起的功能变化

注意 `.stop` 和 `.once` 修饰符一起使用时不会因为顺序产生功能变化，无论是 `.once.stop` 还是 `.stop.once`，元素点击事件只会执行一次且阻止冒泡，此时再次点击元素不会触发点击事件但会冒泡，所以 .stop 和 .once 一次使用只有第一次会阻止冒泡


### 事件传参与事件对象

事件如果同时需要某些参数和事件对象，此时事件对象可以传入 `$event`

```html
<div @click="onClick($event, param)"></div>
```


### 用 render 实现高阶组件

render 函数不熟悉，可以用一个[网站在线转换 template 模板至 render 函数](https://vue-next-template-explorer.netlify.app)
